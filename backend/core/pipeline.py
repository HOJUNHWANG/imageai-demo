...
# ─── Global state ───
pipe = None
PIPE = None  # Legacy alias — kept for compatibility with edit.py references
...
def _apply_optimizations(p, pipeline_name="pipeline", force_cpu_offload=False):
    ...
    if use_cpu_offload:
        ...
        logger.info(f"  [{pipeline_name}] ✓ CPU offload enabled (saves ~4-6GB VRAM)")
    else:
        p.to("cuda")
    ...
    if not use_cpu_offload:
        try:
            p.enable_vae_tiling()
            logger.info(f"  [{pipeline_name}] ✓ VAE tiling enabled")
        except Exception:
            pass

    try:
        p.enable_vae_slicing()
        logger.info(f"  [{pipeline_name}] ✓ VAE slicing enabled")
    except Exception:
        pass
    ...
    try:
        ...
        logger.info(f"  [{pipeline_name}] ✓ Attention slicing enabled")
    except Exception:
        pass
    ...
    try:
        ...
        logger.info(f"  [{pipeline_name}] ✓ xformers enabled")
    except Exception:
        logger.info(f"  [{pipeline_name}] ✓ Flash SDP enabled (xformers not available)")
...
def get_inpaint_pipe():
    ...
    if pipe is not None:
        return pipe
    with _model_lock:
        if pipe is not None:  # Double-check after acquiring lock
            return pipe
        ...
        if txt2img_pipe is not None:
            logger.info("[PIPE] Unloading FLUX Schnell to free VRAM for SDXL...")
            txt2img_pipe = None
            _needs_cleanup = True
        if fill_pipe is not None:
            logger.info("[PIPE] Unloading FLUX Fill to free VRAM for SDXL...")
            fill_pipe = None
            _needs_cleanup = True
        if kontext_pipe is not None:
            logger.info("[PIPE] Unloading FLUX Kontext to free VRAM for SDXL...")
            kontext_pipe = None
            _needs_cleanup = True
        if _needs_cleanup:
            _aggressive_vram_cleanup()
        ...
        logger.info(f"[PIPE] Loading Inpaint from: {model_path}")
        t0 = time.time()
        ...
        try:
            ...
            if COMPILE_UNET and DEVICE == "cuda" and hasattr(torch, "compile"):
                try:
                    ...
                    logger.info("[PIPE] ✓ torch.compile applied to SDXL UNet")
                except Exception as e:
                    logger.warning(f"[PIPE] torch.compile skipped: {e}")

            pipe = p
            PIPE = p
            logger.info(f"[PIPE] SDXL ready in {time.time()-t0:.1f}s")
            return pipe
        except Exception as e:
            logger.error(f"[PIPE] SDXL Load Failed: {e}")
            return None
...
def get_txt2img_pipe():
    ...
    with _model_lock:
        ...
        _needs_cleanup = False
        if pipe is not None:
            logger.info("[PIPE] Unloading SDXL to free VRAM for FLUX Schnell...")
            pipe = None
            PIPE = None
            _needs_cleanup = True
        if fill_pipe is not None:
            logger.info("[PIPE] Unloading FLUX Fill to free VRAM for FLUX Schnell...")
            fill_pipe = None
            _needs_cleanup = True
        if kontext_pipe is not None:
            logger.info("[PIPE] Unloading FLUX Kontext to free VRAM for FLUX Schnell...")
            kontext_pipe = None
            _needs_cleanup = True
        if _needs_cleanup:
            _aggressive_vram_cleanup()
        ...
        try:
            try:
                ...
                logger.info("[FLUX] Loading with 4-bit quantization (NF4)...")
                ...
                logger.info("  [FLUX] ✓ 4-bit quantization + CPU offload (~6GB VRAM)")
            except Exception as quant_err:
                logger.warning(f"  [FLUX] ⚠ 4-bit quantization failed: {quant_err}")
                logger.info("[FLUX] Falling back to bfloat16 with sequential CPU offload...")
                txt2img_pipe = FluxPipeline.from_pretrained(
                    model_id, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True,
                )
                txt2img_pipe.enable_sequential_cpu_offload()
                logger.info("  [FLUX] ✓ Sequential CPU offload (slowest, but fits any GPU)")
...
def get_fill_pipe():
    ...
    with _model_lock:
        ...
        _unload_aux_models()
        if pipe is not None:
            logger.info("[PIPE] Unloading SDXL to free VRAM for FLUX Fill...")
            pipe = None
            PIPE = None
        if txt2img_pipe is not None:
            logger.info("[PIPE] Unloading FLUX Schnell to free VRAM for FLUX Fill...")
            txt2img_pipe = None
        if kontext_pipe is not None:
            logger.info("[PIPE] Unloading FLUX Kontext to free VRAM for FLUX Fill...")
            kontext_pipe = None
        if isinstance(controlnet_pipes, dict):
            controlnet_pipes.clear()
        _aggressive_vram_cleanup()
        ...
        try:
            ...
            try:
                ...
                pass
            fill_pipe = p
            logger.info(f"[FLUX FILL] Ready in {time.time()-t0:.1f}s")
            return fill_pipe
        except Exception as e:
            logger.error(f"[FLUX FILL] Load failed: {e}")
            fill_pipe = None
            return None
...
def get_kontext_pipe():
    ...
    with _model_lock:
        ...
        _unload_aux_models()
        if pipe is not None:
            logger.info("[PIPE] Unloading SDXL to free VRAM for FLUX Kontext...")
            pipe = None
            PIPE = None
        if txt2img_pipe is not None:
            logger.info("[PIPE] Unloading FLUX Schnell to free VRAM for FLUX Kontext...")
            txt2img_pipe = None
        if fill_pipe is not None:
            logger.info("[PIPE] Unloading FLUX Fill to free VRAM for FLUX Kontext...")
            fill_pipe = None
        if isinstance(controlnet_pipes, dict):
            controlnet_pipes.clear()
        _aggressive_vram_cleanup()
        ...
        try:
            ...
            try:
                ...
                pass
            kontext_pipe = p
            logger.info(f"[FLUX KONTEXT] Ready in {time.time()-t0:.1f}s")
            return kontext_pipe
        except Exception as e:
            logger.error(f"[FLUX KONTEXT] Load failed: {e}")
            kontext_pipe = None
            return None
...
def switch_mode(target_mode: str) -> str:
    global CURRENT_MODE
    logger.info(f"[MODE] Switching to {target_mode}...")
...
def get_test_inpaint_pipe(model_id: str):
    """Load a test model as StableDiffusionXLInpaintPipeline for image editing.
    ...
    if test_inpaint_pipe is not None and test_inpaint_pipe_model_id == model_id:
        return test_inpaint_pipe

    model_path = TEST_MODELS.get(model_id)
    if not model_path:
        logger.error(f"[TEST-INPAINT] Unknown model slot: {model_id}")
        return None
    if not os.path.exists(model_path):
        logger.error(f"[TEST-INPAINT] Model file not found: {model_path}")
        return None
    ...
    with _model_lock:
        ...
        t0 = time.time()
        logger.info(f"[TEST-INPAINT] Loading {model_id} from {model_path}")
        dtype = torch.float16 if DEVICE == "cuda" else torch.float32
        try:
            p = StableDiffusionXLInpaintPipeline.from_single_file(
                model_path, torch_dtype=dtype, use_safetensors=True, safety_checker=None,
            )
            p = _apply_optimizations(p, f"TEST-INPAINT/{model_id}")
            test_inpaint_pipe = p
            test_inpaint_pipe_model_id = model_id
            logger.info(f"[TEST-INPAINT] {model_id} ready in {time.time()-t0:.1f}s")
            return test_inpaint_pipe
        except Exception as e:
